...
const WriteAReview = ({ collegeId: propCollegeId, collegeName: propCollegeName, isEmbedded = false, formOnly = false, onCloseForm }) => {
    ...
    const [filterAuthor, setFilterAuthor] = useState(null); // Selected author to follow
    ...
    const topReviewers = React.useMemo(() => {
        ...
        return Object.entries(authorCounts)
            ...
            .map(([author, count]) => ({
                name: author,
                points: count * 50, // Arbitrary point calculation
        ...
        if (match) {
            ...
            if (trendingTags && trendingTags.length > 0) {
                const suggestions = trendingTags
                    .map(t => t.tag)
                    .filter(t => t.toLowerCase().includes(searchStr))
    ...
    const insertTag = (tag) => {
        const cursorPosition = textareaRef.current?.selectionStart || newPostContent.length;
        ...
        const newTextBefore = textBeforeCursor.replace(/#([a-zA-Z0-9_]*)$/, `#${tag} `);
                ...
                Swal.fire({
                    html: `
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding-top: 20px;">
                            <style>
                                @keyframes sweet-smile-bounce {
                                    0% { transform: translateY(0) scale(1) rotate(0deg); }
                                    50% { transform: translateY(-15px) scale(1.15) rotate(10deg); color: #38bdf8; }
                                    100% { transform: translateY(0) scale(1) rotate(0deg); }
                                }
                            </style>
                            <div style="font-size: 85px; color: #0096FF; animation: sweet-smile-bounce 1.2s ease-in-out infinite; text-shadow: 0 10px 25px rgba(0, 150, 255, 0.4);">
                                😃
                            </div>
                            <h2 style="color: #0096FF; font-weight: 800; margin-top: 25px; margin-bottom: 8px; font-family: 'Inter', sans-serif; letter-spacing: -0.5px;">Review Posted!</h2>
        ...
        <div style={{
            ...
            <div style={{
                ...
                <main style={isEmbedded ? { flex: 1, display: 'flex', flexDirection: 'column', height: formOnly ? 'auto' : '600px' } : { flex: 1 }}>
                                            ...
                                            <div style={{
                                                ...
                                                {tagSuggestions.map((tag, idx) => (
                                                    <div
                                                        ...
                                                        style={{ padding: '10px 16px', cursor: 'pointer', fontSize: '14px', fontWeight: 600, color: colors.textDark, display: 'flex', alignItems: 'center', gap: '10px', transition: 'background 0.2s' }}
                                                        className="hover-bg-light"
                                                    >
                    ...
                    {!formOnly && (
                        <div style={isEmbedded ? { flex: 1, overflowY: 'auto', paddingRight: '8px' } : { height: 'calc(100vh - 180px)', overflowY: 'auto', paddingRight: '12px' }} className="hide-scroll-bar">
